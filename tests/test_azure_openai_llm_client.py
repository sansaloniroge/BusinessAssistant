...
async def test_generate_returns_text_and_usage():
    client = _FakeAzureOpenAIClient(text="hola [C1]")
    llm = AzureOpenAILLMClient(client=client, deployment="my-gpt-deployment")
...
@pytest.mark.asyncio
async def test_generate_calls_azure_using_the_deployment_name_not_the_model_family():
    client = _FakeAzureOpenAIClient()
    llm = AzureOpenAILLMClient(client=client, deployment="my-gpt-deployment")

    await llm.generate(system="sys", user="pregunta", context="", model="gpt-4.1-mini")

    call = client.chat.completions.calls[0]
    # Azure enruta por deployment, no por familia de modelo -- "gpt-4.1-mini"
    # (el `model` que pasa el caller) nunca debería llegar a la llamada real.
    assert call["model"] == "my-gpt-deployment"
...
async def test_generate_applies_base_class_timeout():
    client = _FakeAzureOpenAIClient(sleep_s=0.05)
    llm = AzureOpenAILLMClient(client=client, deployment="my-gpt-deployment")
...
def test_missing_deployment_without_client_raises(monkeypatch):
    monkeypatch.delenv("AZURE_OPENAI_ENDPOINT", raising=False)
    monkeypatch.delenv("AZURE_OPENAI_API_KEY", raising=False)
    monkeypatch.delenv("AZURE_OPENAI_DEPLOYMENT", raising=False)

    with pytest.raises(ValueError, match="AZURE_OPENAI_ENDPOINT.*AZURE_OPENAI_API_KEY.*AZURE_OPENAI_DEPLOYMENT"):
        AzureOpenAILLMClient()


def test_missing_deployment_with_explicit_client_raises(monkeypatch):
    monkeypatch.delenv("AZURE_OPENAI_DEPLOYMENT", raising=False)

    with pytest.raises(ValueError, match="AZURE_OPENAI_DEPLOYMENT"):
        AzureOpenAILLMClient(client=_FakeAzureOpenAIClient())
